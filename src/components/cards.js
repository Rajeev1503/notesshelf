              ...
              >
                <Link href={`/${e.subject.slug}/${e.slug}`}>
                  <div className="flex lg:flex-row-reverse flex-col-reverse md:items-center lg:justify-between lg:gap-8 px-4 lg:px-8">
                    ...
                    <div className="xl:w-[70%] lg:w-[60%] h-min flex flex-col gap-2 rounded-lg overflow-hidden font-semibold">
                      <div
                        className={`${backgroundColorContext.backgroundColorState.main_text} h-min text-[1.4rem]`}
                      >
                        <p>{e.title}</p>
                      </div>
                      <div
                        className={`${backgroundColorContext.backgroundColorState.gray_text} text-[0.92rem]`}
                      >
                        {e?.excerpt ? (
                          <p>{e.excerpt}</p>
                        ) : (
                ...
                <div className="pb-2 flex flex-row justify-start items-center gap-2 text-center text-xs font-semibold rounded-lg px-4 lg:px-8">
                  {props.isLatestPage && (
                    <>
                      <div
                        className={`flex-grow max-w-max py-1 px-2 rounded-lg cursor-pointer ${backgroundColorContext.backgroundColorState.main_text} ${backgroundColorContext.backgroundColorState.card_background}`}
                      >
                  ...
                  <div
                    className={`flex-grow max-w-max py-1 px-2 rounded-lg ${backgroundColorContext.backgroundColorState.main_text} ${backgroundColorContext.backgroundColorState.card_background}`}
                  >
                    <div>
                      {isSaved ? (
                        ...
                        <span
                          ...
                          onClick={() => savePostHandler(e.slug)}
                        >
                          <TbPlaylistAdd />
                          save
                        </span>
                  ...
                  >
                    <div
                      className={`max-w-max ${backgroundColorContext.backgroundColorState.main_text} ${backgroundColorContext.backgroundColorState.card_background} py-1 px-2 rounded-lg`}
                    >
